...
Slash commands and a skill over the [Levirge Search](https://search.levirge.com) MCP server:
web search via SearXNG, and a routed fetch that escalates to a stealth browser for sites that
block ordinary crawling. Commands install as `/search:research`, `/search:fetch`,
`/search:stealth`.
...
Then restart the client. The desktop "Add marketplace" dialog accepts `levirge/search` too — it
only allows github.com, gitlab.com, bitbucket.org and GitHub Enterprise hosts, which is why this
repo exists rather than the self-hosted marketplace the service also serves.
...
On Codex, bump the version and reinstall, then open a **new** task — refreshed `interface`
metadata (name, logo, icon) does not appear in an already-running one.
...
The server requires authentication — **there is no anonymous access.** Two ways in:

- **OAuth (recommended)** — leave the plugin's `token` field blank. `/mcp` answers an RFC 9728
  challenge pointing at `auth.levirge.com`, and any client that speaks MCP authorization runs
  sign-in for you. This is the path the spec-conformant `mcp.json` uses.
- **Bearer token** — mint one in the app under **Profile → tokens** and paste it into the
  plugin's `token` user-config field. Shown once; it cannot be recovered.
...
repository under `plugin/` and copied here — **edit it there**, not here, or the next sync will
